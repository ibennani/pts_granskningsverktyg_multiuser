...
**New view components:** Use `export class` with `init({ root, deps })`, `render()`, `destroy()`. Register one instance per view in `js/logic/view_components_index.js`. See `docs/component_standard.md` and `AGENTS.md`.

**Legacy section components** may still use `export const` object literals (no IIFE). Load CSS via `Helpers.load_css_safely()` in `init()`.
