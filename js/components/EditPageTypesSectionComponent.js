...
export const EditPageTypesSectionComponent = {
    ...
    async init({ root, deps }) {
        ...
        if (this.Helpers?.load_css) {
            await this.Helpers.load_css(this.CSS_PATH).catch(err => { if (window.ConsoleManager) window.ConsoleManager.warn('[EditPageTypesSectionComponent] Failed to load CSS', err); });
        }
    },
