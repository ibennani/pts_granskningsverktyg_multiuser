/**
 * @fileoverview Knappar som återställer filter i granskningslistan.
 */
...
/**
 * Primär återställ: rensar bara sekundära filter (behåller sök).
 * @param {object} ctx
 * @param {number} secondary_count
 * @param {boolean} panel_open
 * @returns {HTMLButtonElement}
 */
export function create_audit_filter_primary_reset_button(ctx, secondary_count, panel_open) {
    const t = ctx.get_t_func();
    const show = secondary_count > 0 && !panel_open;
    const reset_btn = ctx.Helpers.create_element('button', {
        class_name: ['button', 'button-default', 'audit-filter-reset', 'audit-filter-primary-reset'],
        attributes: {
            type: 'button',
            'aria-label': t('audit_filter_reset_secondary_aria')
        }
    });
    reset_btn.hidden = !show;
    if (!show) {
    ...
    reset_btn.appendChild(
        ctx.Helpers.create_element('span', {
            class_name: 'audit-filter-reset__text',
            text_content: t('audit_filter_reset_secondary')
        })
    );
    const icon = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    icon.setAttribute('viewBox', '0 0 24 24');
    ...
    icon.appendChild(path);
    reset_btn.appendChild(icon);
    reset_btn.addEventListener('click', ctx.handle_audit_filter_reset_secondary);
    ctx._auditFilterPrimaryResetRef = reset_btn;
    return reset_btn;
}
