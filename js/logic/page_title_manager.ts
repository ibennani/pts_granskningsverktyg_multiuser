...
/** Delmängd av app-state som behövs för sidtitel. */
export type PageTitleAppState = {
    auditStatus?: string;
    auditMetadata?: { actorName?: string };
    uiSettings?: {
        requirementAuditSidebar?: { selectedMode?: string };
    };
    samples?: Array<{ id?: string | number; description?: string }>;
    ruleFileContent?: {
        requirements?: Record<string, RulefileRequirement | undefined>;
    } | null;
};

/** Route-parametrar (hash/query) för titelbygge. */
export type PageTitleRouteParams = {
    section?: string;
    editSection?: string;
    type?: string;
    sampleId?: string;
...
type TFunction = (key: string, replacements?: Record<string, unknown>) => string;

type TranslationLike = { t: TFunction } | null | undefined;

/**
 * Hittar kravobjekt i regelfilens innehåll utifrån route-param (id/nyckel).
 */
function find_requirement_in_rulefile_content(
    current_state: PageTitleAppState,
    req_id: string
): RulefileRequirement | null {
    if (!req_id) return null;
    const requirements_map = current_state?.ruleFileContent?.requirements || {};
    const direct = requirements_map[req_id];
    if (direct) return direct;
    const found = Object.values(requirements_map).find((req) => {
...
function get_rulefile_edit_requirement_title_prefix(
    params: PageTitleRouteParams,
    current_state: PageTitleAppState,
    t: TFunction
): string {
    ...
    const name_part =
        (requirement && String(requirement.title ?? '').trim()) || (req_id || '');
    return name_part ? `${edit_label} | ${name_part}` : edit_label;
}

function get_rulefile_view_requirement_title_prefix(
    params: PageTitleRouteParams,
    current_state: PageTitleAppState,
    t: TFunction
): string {
    const view_label = t('rulefile_requirements_menu_title');
    const req_id = params?.id != null ? String(params.id) : '';
    const requirement = find_requirement_in_rulefile_content(current_state, req_id);
    const name_part =
        (requirement && String(requirement.title ?? '').trim()) || (req_id || '');
    return name_part ? `${view_label} | ${name_part}` : view_label;
}

function get_t_fallback(Translation: TranslationLike): TFunction {
    return Translation && typeof Translation.t === 'function'
        ? Translation.t.bind(Translation)
        : (key: string, _replacements?: Record<string, unknown>) => `**${key}**`;
}

export type PageTitleContext = {
    getState: () => PageTitleAppState;
    Translation: TranslationLike;
};

export function get_page_title_prefix(
    view_name: string,
    params: PageTitleRouteParams,
    { getState, Translation }: PageTitleContext
): string {
    ...
    try {
        if (audit_status === 'rulefile_editing') {
            ...
            if (view_name === 'rulefile_sections' && section_to_menu_key[section]) {
                ...
                title_prefix = get_rulefile_edit_requirement_title_prefix(
                    params,
                    current_state,
                    t
                );
            } else if (view_name === 'rulefile_view_requirement') {
                title_prefix = get_rulefile_view_requirement_title_prefix(
                    params,
                    current_state,
                    t
                );
            } else if (
                ['rulefile_requirements', 'rulefile_add_requirement'].includes(view_name)
            ) {
                title_prefix = t('rulefile_requirements_menu_title');
            } else if (view_name === 'rulefile_metadata_edit') {
                title_prefix = t(
                    section && section_to_menu_key[section]
                        ? section_to_menu_key[section]
                        : 'rulefile_section_general_title'
                );
                ...
                view_name === 'confirm_delete' &&
                (params.type === 'check' || params.type === 'criterion')
            ) {
                title_prefix = t('rulefile_requirements_menu_title');
        ...
        if (title_prefix === t('app_title')) {
            switch (view_name) {
                case 'start':
                    title_prefix = t('menu_link_manage_audits');
                    break;
                case 'audit':
                    title_prefix = t('audit_title');
                    break;
                case 'audit_audits':
                    title_prefix = t('audit_title_audits');
                    break;
                case 'audit_rules':
                    title_prefix = t('audit_title_rules');
                    break;
                case 'manage_users':
                ...
                case 'my_settings':
                    ...
                    break;
                case 'statistics':
                    title_prefix = t('menu_link_statistics');
                    break;
                case 'login':
                ...
                case 'backup_settings':
                    title_prefix = t('menu_link_backups');
                    break;
                case 'confirm_delete':
                    ...
                    else if (params.type === 'criterion') {
                        title_prefix = t('confirm_delete_criterion_title');
                    }
                    break;
                case 'requirement_audit': {
                    const sidebar_mode = current_state?.uiSettings?.requirementAuditSidebar
                        ?.selectedMode;
                    if (sidebar_mode === 'requirement_samples') {
                        const sample = (current_state?.samples || []).find(
                            (s) => String(s?.id) === String(params.sampleId || '')
                        );
                        ...
                        let requirement: RulefileRequirement | null | undefined =
                            requirements_map?.[req_id];

                        if (!requirement && req_id) {
                            requirement =
                                Object.values(requirements_map).find((req) => {
                                    if (!req || typeof req !== 'object') return false;
                                    const key =
                                        req.key !== undefined && req.key !== null
                                            ...
                                            : '';
                                    const id =
                                        req.id !== undefined && req.id !== null ? String(req.id) : '';
                                    return key === String(req_id) || id === String(req_id);
                                }) || null;
                        }
                        if (!requirement && req_id) {
                            consoleManager.warn(
                                '[page_title_manager] build_page_title: requirement not found for requirement_audit',
                                {
...
export function build_page_title(
    view_name: string | undefined,
    params: PageTitleRouteParams | undefined,
    { getState, Translation }: PageTitleContext
): string {
    const t = get_t_fallback(Translation);
    ...
    const current_state = getState();
    const audit_status = current_state?.auditStatus;
    const vn = view_name || 'start';
    const p = params || {};
    const title_prefix = get_page_title_prefix(vn, p, { getState, Translation });

    let final_title = `${title_prefix}${title_suffix}`;
    const is_inside_audit =
        audit_status !== 'rulefile_editing' &&
        !['start', 'audit', 'audit_audits', 'audit_rules', 'login', 'manage_users', 'my_settings', 'statistics'].includes(
            String(view_name)
        );
    const actor_name =
        is_inside_audit && current_state?.auditMetadata?.actorName
...
export function updatePageTitle(
    ...
    if (!new_title || !String(new_title).trim()) {
        consoleManager.warn(
            '[page_title_manager] updatePageTitle: build_page_title returnerade tom sträng. view:',
            view_name,
            'params:',
...
export type UpdatePageTitleFromCurrentViewArgs = {
    getState: () => PageTitleAppState;
    Translation: TranslationLike;
    get_current_view_name: () => string | undefined | null;
    get_current_view_params_json: () => string | undefined | null;
};

export function updatePageTitleFromCurrentView({
    getState,
    Translation,
    get_current_view_name,
    get_current_view_params_json
}: UpdatePageTitleFromCurrentViewArgs): void {
    let params: PageTitleRouteParams = {};
    try {
        params = JSON.parse(get_current_view_params_json() || '{}') as PageTitleRouteParams;
    } catch {
