    ...
    cleanup() {
        ...
        for (const callback of this.cleanupCallbacks) {
            try {
                ...
                if (window.ConsoleManager?.warn) window.ConsoleManager.warn('Error in cleanup callback:', error);
            }
        }
...
if (process.env.NODE_ENV === 'development' || window.location.hostname === 'localhost') {
    setInterval(() => {
        ...
        if (warnings.length > 0) {
            if (window.ConsoleManager?.warn) window.ConsoleManager.warn('Potential memory leaks detected:', warnings);
        }
