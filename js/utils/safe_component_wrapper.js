...
export function createSafeComponent(component, requiredDependencies = []) {
    const safeComponent = {
        ...
        async init(...args) {
            ...
            for (const depName of requiredDependencies) {
                if (!dependencyManager.isDependencyAvailable(depName)) {
                    if (window.ConsoleManager?.warn) window.ConsoleManager.warn(`SafeComponent: Required dependency '${depName}' not available`);
        ...
        render(...args) {
            ...
            for (const depName of requiredDependencies) {
                if (!dependencyManager.isDependencyAvailable(depName)) {
                    if (window.ConsoleManager?.warn) window.ConsoleManager.warn(`SafeComponent: Required dependency '${depName}' not available during render`);
                    return;
                }
            }
...
export function safeAssignDependencies(target, dependencyNames) {
    ...
    for (const depName of dependencyNames) {
        ...
        if (value) {
            ...
            if (window.ConsoleManager?.warn) window.ConsoleManager.warn(`SafeAssignDependencies: Dependency '${depName}' not available`);
            allAssigned = false;
        }
    }
