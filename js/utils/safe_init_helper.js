...
export async function safeAssignGlobals(target, dependencyNames, maxRetries = 10, retryDelay = 50) {
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        ...
        if (attempt < maxRetries - 1) {
            if (window.ConsoleManager?.warn) window.ConsoleManager.warn(`SafeAssignGlobals: Attempt ${attempt + 1} failed, retrying in ${retryDelay}ms...`);
            await new Promise(resolve => setTimeout(resolve, retryDelay));
        }
    }
    
    if (window.ConsoleManager?.warn) window.ConsoleManager.warn('SafeAssignGlobals: Failed to assign all dependencies after maximum retries');
    return false;
}
...
function getDependencyValue(depName) {
    ...
    if (getter) {
        try {
            ...
            if (window.ConsoleManager?.warn) window.ConsoleManager.warn(`SafeInitHelper: Failed to get dependency '${depName}':`, error);
        }
    }
    ...
    for (const [name, getter] of Object.entries(dependencyMap)) {
        try {
            ...
            if (value) {
                ...
                if (window.ConsoleManager?.warn) window.ConsoleManager.warn(`SafeAssignDependencies: Dependency '${name}' not available`);
                allAssigned = false;
            }
        } catch (error) {
            if (window.ConsoleManager?.warn) window.ConsoleManager.warn(`SafeAssignDependencies: Error getting dependency '${name}':`, error);
            allAssigned = false;
        }
    }
