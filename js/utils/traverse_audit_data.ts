...
/** Kontext när man traverserar kriterier inom ett redan valt kravresultat. */
export type PassCriteriaInRequirementContext = {
    check_key: string;
    check_result: CheckResultNode;
    pc_key: string;
    pc_result: PassCriterionResult;
...
function walk_pass_criteria_for_sample (
    sample: SampleNode,
...
/**
 * Besöker alla bedömningskriterier under ett enskilt kravresultat (t.ex. bristindex per krav).
 */
export function traverse_pass_criteria_in_requirement_result (
    req_result: RequirementResultNode | null | undefined,
    callback: (ctx: PassCriteriaInRequirementContext) => void
): void {
...
export function traverse_all_pass_criteria (
    ...
    for (const sample of audit_state?.samples ?? []) {
        walk_pass_criteria_for_sample(sample, callback);
    }
}
