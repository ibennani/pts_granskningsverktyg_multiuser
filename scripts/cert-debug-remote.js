...
async function run() {
    try {
        console.info('--- /etc/nginx/ssl (lista) ---');
        ...
        await exec(
            ...
            + "console.info(JSON.stringify({subject:c.subject,issuer:c.issuer,subjectaltname:c.subjectaltname,valid_from:c.valid_from,valid_to:c.valid_to,issuerCertificate: c.issuerCertificate? {subject:c.issuerCertificate.subject,issuer:c.issuerCertificate.issuer}: null},null,2));"
            + "s.end();"
            + "});"
