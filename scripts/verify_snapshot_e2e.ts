...
async function main(): Promise<void> {
    ...
    try {
        ...
        const visible = await start_snapshot_capture(ctx.audit_id, {
        ...
        const row = await get_audit_snapshot_by_id(ctx.audit_id, capture_id);
        if (!row || row.status !== 'ready') {
            throw new Error(
                `Förväntade status ready, fick ${row?.status ?? 'saknas'}: ${row?.error ?? 'okänt fel'}`
            );
        }
            ...
            JSON.stringify(
                {
                    ...
                    page_title: visible.pageTitle.value ?? row.page_title,
