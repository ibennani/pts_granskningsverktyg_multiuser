...
export function collapse_bilingual_requirement_title(title) {
    ...
    const sw_after_english =
        /\s(Tre|Två|Fyra|Minsta|Etiketter|Synligt|Synliga|Dold|Dolt|Ingen|Inga|Tillräckligt|Förutsägbar|Tydlig|Tydliga|Konsekvent|Fokus|Rubrik|Rubriker|Ordning|Uppmärksamhet|Åter|Ändring|Över)(?=\s|$)/u;
...
/**
 * Visningsnamn för statistik: endast kravtexten (titel), utan standardReference/nyckel före.
...
export function requirement_stats_display_name(req) {
    ...
    const title = collapse_bilingual_requirement_title(String(req.title ?? '').trim());
    if (title) return title;

    const ref = (req.standardReference?.text && String(req.standardReference.text).trim()) || '';
...
/**
 * @param {number} year
 * @param {{ durations: number[], by_monitoring: Map<string, object>, principle_scores?: Record<string, number[]>, total_scores?: number[] }} bucket
...
export function build_statistics_from_audit_rows(rows) {
    /** @type {Map<number, { durations: number[], by_monitoring: Map<string, { audit_count: number, fail_counts: Map<string, number>, req_defs: Map<string, object|null> }>, principle_scores: Record<string, number[]>, total_scores: number[] }>} */
    ...
    for (const row of rows) {
        ...
        const label = get_monitoring_type_label(rule);
        const mon = ensure_monitoring_bucket(yb.by_monitoring, label);
        mon.audit_count += 1;
